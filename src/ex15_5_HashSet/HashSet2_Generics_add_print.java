...
/*
 	Set 인터페이스
 		- HashSet (상속받는 클래스) : 내부적으로 HashMap을 이용해서 만들어졌으며
 		  HashSet이란 이름은 해싱을 이용하여 구현했기 때문에 붙여진 것입니다.
 	...
 	1. 여러가지 자료형(기본자료형, 참조형 모두 )의 Data를 모두 저장할 수 있습니다.
 		  ex) int, double, charm boolean, String etc
 	2. 순서 없이 입, 출력합니다.
 	3. 중복된 Data를 저장하지 못합니다.
 		  ex)로또 번호6개를 중복 되지 않게 하는 프로그램시 이용
 */
public class HashSet2_Generics_add_print {
	...
	public static void main(String[] args) {
		...
		set.add("에스프레소");
		set.add("자바");
		System.out.println("저장된 데이터에 수 = " + set.size());

		// set 객체가 보관하고 있는 데이터 객체 출력
		System.out.println(set);

		// iterator(): set 에 있는 데이터를 모두 가져옵니다.
